...
describe("serviceQuoteEntrySchema", () => {
  ...
  it("accepts an optional dry-cleaning notes string", () => {
    expect(
      serviceQuoteEntrySchema.safeParse({
        dryCleaningItemSubtotalCents: 1800,
        dryCleaningNotes: "1 stain on collar, extra starch requested",
        sameDayApproved: false,
      }).success
    ).toBe(true);
  });
...
describe("buildServiceQuoteUpdatePayload", () => {
  ...
  it("writes the dry-cleaning subtotal, effective charge, and notes together, never one without the others", () => {
    ...
    expect(dryOnly).toHaveProperty("dry_cleaning_notes");
  });

  it("stores the entered dry-cleaning notes", () => {
    const payload = buildServiceQuoteUpdatePayload(
      "dry_cleaning",
      "dry_cleaning_timeline",
      {
    ...
    expect(payload.dry_cleaning_notes).toBe("1 stain on collar, extra starch requested");
  });

  it("stores null dry-cleaning notes when none were entered", () => {
    const payload = buildServiceQuoteUpdatePayload(
      "dry_cleaning",
      "dry_cleaning_timeline",
      { dryCleaningItemSubtotalCents: 1800, sameDayApproved: false },
    ...
    expect(payload.dry_cleaning_notes).toBeNull();
  });

  it("omits dry_cleaning_notes for a wash_and_fold-only quote", () => {
    const payload = buildServiceQuoteUpdatePayload(
      "wash_and_fold",
      "standard",
      { actualWeightLb: 20, sameDayApproved: false },
    ...
    expect(payload).not.toHaveProperty("dry_cleaning_notes");
  });
