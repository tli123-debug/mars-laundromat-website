...
/**
 * Formats a stored "YYYY-MM-DD" date as a short display string, e.g. "Fri,
 * Sep 25" — shared between the public form's own inline date displays and
 * buildRequestedScheduleSummary() (src/lib/booking-submission-storage.ts)
 * so both use exactly the same formatting.
 */
...
// Shape only — no date-sensitive or cross-field business rules. Used by
// createBooking()'s recovery check (find_existing_booking_submission),
// which must be able to look up an already-accepted booking even after its
// originally-requested date has rolled into the past; the superRefine
// below (bookingSchema) is where "pickup date can't be in the past" and
// every other business rule actually lives, and must never gate recovery.
export const bookingSchemaShape = z.object({
...
export const bookingSchema = bookingSchemaShape.superRefine((data, ctx) => {
